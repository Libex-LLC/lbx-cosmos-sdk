...
func getNewInitChainer(mapp *mock.App, keeper Keeper) sdk.InitChainer {
	return func(ctx sdk.Context, req abci.RequestInitChain) abci.ResponseInitChain {
		...
		stakeGenesis.Params.BondDenom = "LBX"
...
func getNewStakeMockApp(t *testing.T) (*mock.App, Keeper, []Account) {
	...
	bondCoin := sdk.NewCoin("LBX", sdk.NewDecWithoutFra(10).RawInt())
...
func GenAccounts(n int) (accounts []Account) {
	for i := 0; i < n; i++ {
		...
		genCoin := sdk.NewCoin("LBX", sdk.NewDecWithoutFra(12345678).RawInt())
		baseAccount := auth.BaseAccount{
			Address: address,
...
func TestNewStake(t *testing.T) {
	...
	for i := 2; i < 12; i++ {
		newBondCoin := sdk.NewCoin("LBX", sdk.NewDecWithoutFra(20000+int64(i)).RawInt())
